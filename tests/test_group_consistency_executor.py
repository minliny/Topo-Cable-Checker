...
def _make_context(parameter_profile=None, threshold_profile=None):
    return {
...
class TestEmptyAndDegenerateInputs:
    ...
    def test_missing_target_type_returns_no_issues(self):
        ...
        result = executor.execute(compiled, {"links": []}, _make_context())
        assert result == []
    ...
    def test_parameter_key_not_found_falls_back_to_inline(self):
        ...
        result = executor.execute(compiled, {"devices": devices}, _make_context())
        assert len(result) == 1
        assert result[0].category == "execution_error"
